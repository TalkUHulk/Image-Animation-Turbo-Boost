...
def cof1(M, i, j):
    ...
    xs = torch.cat((s, x), axis=0)

    return xs

    # return M[1 - i - 1, 1 - j - 1]


def alcof(M, i, j):
    return torch.pow(torch.tensor(-1), i + j) * cof1(M, i, j)
...
def adj(M):
    ...
    for i in range(1, M.shape[0] + 1):
        for j in range(1, M.shape[1] + 1):
            result[j - 1][i - 1] = torch.pow(torch.tensor(-1), i + j) #* M[1 - i - 1, 1 - j - 1]
    M = M.flatten().flip(0).view(2, 2).t()
    result = result * M
    return result
# def adj(M):
#     result = torch.zeros((M.shape[0], M.shape[1]), device=M.device)
...
class DenseMotionNetwork(nn.Module):
    ...
    def create_sparse_motions(self, source_image, kp_driving_value, kp_driving_jacobian,
        ...
        Eq 4. in the paper T_{s<-d}(z)
        """
        bs, _, h, w = source_image.shape  # torch.Size([1, 3, 64, 64])
        ...
        """
        coordinate_grid torch.Size([1, 10, 64, 64, 2])
        jacobian torch.Size([1, 10, 64, 64, 2, 2])
        """
        # jacobian = torch.matmul(kp_source_jacobian, torch.inverse(kp_driving_jacobian))
        jacobian = torch.matmul(kp_source_jacobian,
                                torch.stack([invmat(kp_driving_jacobian[0, i]) for i in range(10)], 0).unsqueeze(0))
    ...
    def forward(self, source_image, kp_driving_value, kp_driving_jacobian, kp_source_value, kp_source_jacobian):
        ...
        # Sec. 3.2 in the paper
        occlusion_map = torch.sigmoid(self.occlusion(prediction))
        # return deformed_source, mask, torch.ones([1, 64, 64, 2]), occlusion_map
